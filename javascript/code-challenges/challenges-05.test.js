...
const toLastNames = (people) => {
  // Solution code here...
  let name = [{firstName:"Jane", lastName:"Doe"}, {firstName:"James", lastName:"Bond"}];
  return people.map((person) => person.name);
};
console.log(toLastNames)
...
const addPurchases = (arr) => {
  // Solution code here...
  let purchasePrice = 399
  return arr.reduce((item, n) => {
    item + n
  })
};
...
const returnNames = (arr) => {
  // Solution code here...
  return arr.reduce((sum, starWarsData) => {
    console.log(sum)
    return sum + starWarsData.name
  })
};
